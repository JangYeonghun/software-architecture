...
public class Info extends JFrame {
    ...
    Info() {        
        ...
        setVisible(true);
        //상영 영화 정보 변경 버튼
        JButton change = new JButton("상영 영화 관리");
        change.setFont(new Font("한컴 말랑말랑 Bold", Font.BOLD, 12));
        change.setLocation(550, 0);
        ...
        change.addActionListener((ActionEvent e) -> {
            ...
            int option = JOptionPane.showConfirmDialog(null, passwordField, "비밀번호를 입력하세요", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
            if (option == JOptionPane.OK_OPTION){
                char [] passwordChars = passwordField.getPassword();
                String inputPassword = new String(passwordChars);
            
                if (inputPassword != null && inputPassword.equals(PASSWORD)) {
                    MovieInfoFacade facade = new MovieInfoFacade();
                    setVisible(false);
                } 
                else {
